...
import { atomCexAccountFieldsAreLocked } from '../../../libs/store/cex-accoiunt-debounce-name.atom.ts';
import {
  useCexJwtsStore,
  useJwtStore,
...
export const useAddCexAccount = () => {
  ...
  useEffect(() => {
    ...
    if (createdResp?.jwtToken && createdResp?.meta?.id) {
      setJwtCexAccount(createdResp.meta.id, createdResp.jwtToken);
